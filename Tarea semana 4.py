...
for degree in degrees:
    ...
    for idx, C in enumerate(C_values, 1):
        ...
        Z = y_pred.reshape(X_grid.shape)

        # Graficar
        ax.plot_surface(X_grid, Y_grid, Z, alpha=0.7, cmap='viridis')

        ax.scatter(X[:, 0], X[:, 1], y, c='r', marker='o', label='Datos de entrenamiento')
...
for alpha in alphas:
    ...
    scores = cross_val_score(lasso, X_poly_2, y, cv=5, scoring='neg_mean_squared_error')
...
for alpha in alphas:
    ...
    scores_ridge = cross_val_score(ridge, X_poly_2, y, cv=5, scoring='neg_mean_squared_error')
...
for degree in degrees:
    poly_1 = PolynomialFeatures(degree=degree)
    X_test_poly_1 = poly_1.fit_transform(X_test)

    for idx, C in enumerate(C_values, 1):
        ax = fig.add_subplot(len(degrees), len(C_values), (degree - 1) * len(C_values) + idx, projection='3d')

        # Realizar predicciones con Ridge
        y_pred = ridge_models[(degree, C)].predict(X_test_poly_1)
        Z = y_pred.reshape(X_grid.shape)

        # Graficar la superficie de predicciones
        ax.plot_surface(X_grid, Y_grid, Z, alpha=0.7, cmap='viridis')
